...
namespace MF.NetCoreApp
{
    ...
    public class ApiBaseController : ControllerBase
    {
        ...
        /// <returns></returns>
        [NonAction]
        public async Task<IActionResult> ExportExcel(PubResponse resp, string outpath, int rowstart = 0, string sheetname = "sheet")
        {
            IWorkbook workbook;
            dynamic list = ((dynamic)resp.Data);
            if (list == null || list.Count == 0)
            {
                throw new Exception("没有记录，不能导出");
            }
            workbook = NpoiUtil.ExportToExcelReport(list, rowstart, sheetname);
