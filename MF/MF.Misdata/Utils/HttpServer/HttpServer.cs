    ...
    public class HttpServer
    {
        ...
        private async void GetConnectCallBack(IAsyncResult ar)
        {
            if (ar.AsyncState is HttpListener listener)
            {
                ...
                {
                    logNet?.WriteException(ToString(), ex, "common");
                }
                ...
                {
                    logNet?.WriteException(ToString(), $"Handle Request[{request.HttpMethod}], {request.RawUrl}", ex, "common");
                }
            }
        }
        ...
        public Dictionary<string, MqttRpcApiInfo> apiTopicServiceDict;
        ...
        public void RegisterHttpRpcApi(string api, object obj)
        {
            lock (rpcApiLock)
            {
                ...
                {
                    if (apiTopicServiceDict.ContainsKey(item.ApiTopic))
                    {
                        apiTopicServiceDict[item.ApiTopic] = item;
                    }
                    else
                    {
